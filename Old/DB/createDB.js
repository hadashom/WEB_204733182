...
const createTableAndInsertDataIntoMembers = async (req, res) => {
    try {
        // create members table
        let query = "CREATE TABLE IF NOT EXISTS members (email varchar(255) PRIMARY KEY NOT NULL, fname varchar(255) NOT NULL, lname varchar(255) NOT NULL, password varchar(255) NOT NULL, question1 int, question2 int, question3 int, question4 int, question5 int, totalScore double(10,2))";
...
const showMembers = async (req, res) => {
    try {
        ...
        const mysqlres = await sql.query(query);
        res.send(mysqlres);
    } catch (err) {
        console.log("error in showing users ", err);
        res.send("error in showing users " + err);
    }
}
...
const createAndPopulateStocksTable = async (req, res) => {
    try {
        // Create table
        let query = "CREATE TABLE IF NOT EXISTS stocks (stock VARCHAR(255) NOT NULL PRIMARY KEY, month6change FLOAT(5,2), month12change FLOAT(5,2), month24change FLOAT(5,2), volatilityIndex FLOAT(5,2))";
        await sql.query(query);
        ...
        fs.createReadStream(filePath)
            .pipe(csv({ bom: true }))
            .on('data', (data) => {
                stocks.push([data.stock, data.month6change, data.month12change, data.month24change, data.volatilityIndex]);
            })
            .on('end', async () => {
                // Insert data into table
                let sqlQuery = "INSERT INTO stocks (stock, month6change, month12change, month24change, volatilityIndex) VALUES ?";
                let result = await sql.query(sqlQuery, [stocks]);
...
module.exports = { createTableAndInsertDataIntoMembers, showMembers, dropTables, createAndPopulateStocksTable };
